...
app.use("/returns", express.static(path.join(__dirname, "returns", "public"), {
  etag: false,
  setHeaders: (res, fp) => { if (String(fp).endsWith(".html")) res.setHeader("Cache-Control", "no-store"); }
}));


// Explicit portal index (works even when static files are bundled in serverless)
app.get(["/", "/index.html"], (req, res) => {
  res.setHeader("Cache-Control", "no-store");
  const p = path.join(__dirname, "public", "index.html");
  if (!fs.existsSync(p)) return res.status(404).send("Portal index missing");
...
app.get(["/returns", "/returns/"], (req, res) => {
  res.setHeader("Cache-Control", "no-store");
  const p = path.join(__dirname, "returns", "public", "index.html");
  if (!fs.existsSync(p)) return res.redirect("/");
  return res.sendFile(p);
});
app.get("/api/meta", (req, res) => {
  res.json({
    ...
    port: PORT,
    ips: getLanIps(),
    paths: { portal: "/", it: "/it/login.html", universe: "/universe/index.html", returns: "/returns/index.html" },
...
async function initDB(client) {
  ...
  // --- Returns schema upgrades (safe to run multiple times) ---
  const _alter = async (sql) => { try { await client.execute({ sql, args: [] }); } catch {} };
  await _alter("ALTER TABLE it_returns ADD COLUMN returner_name TEXT");
  await _alter("ALTER TABLE it_returns ADD COLUMN returner_email TEXT");
  await _alter("ALTER TABLE it_returns ADD COLUMN department TEXT");
  await _alter("ALTER TABLE it_returns ADD COLUMN location TEXT");
  await _alter("ALTER TABLE it_returns ADD COLUMN accessories TEXT");
  await _alter("ALTER TABLE it_returns ADD COLUMN condition_detail TEXT");
  await _alter("ALTER TABLE it_returns ADD COLUMN completed_at TEXT");
  await _alter("ALTER TABLE it_returns ADD COLUMN received_by TEXT");
...
// ================= Equipment Returns (Admin-only, separate UI at /returns) =================
itApi.get("/returns", itAuth, itRequireRole("admin"), async (req, res) => {
  await ensureDbReady();
  ...
  if (status) { where.push("upper(r.status)=upper(?)"); args.push(status); }
  if (q) {
    where.push(`(
      coalesce(r.asset_tag,'') LIKE ? OR
      coalesce(r.item_name,'') LIKE ? OR
      coalesce(r.serial_no,'') LIKE ? OR
      coalesce(r.returner_name,'') LIKE ? OR
      coalesce(r.returner_email,'') LIKE ? OR
      coalesce(r.department,'') LIKE ?
    )`);
    const like = `%${q}%`;
...
itApi.get("/returns/stats", itAuth, itRequireRole("admin"), async (req, res) => {
  await ensureDbReady();
  ...
  const byDay = await dbAll(
    `SELECT date(created_at) as day, COUNT(*) as count
     FROM it_returns
     WHERE is_deleted=0 AND created_at >= datetime('now','localtime', ?)
  ...
  const byDepartment = await dbAll(
    `SELECT coalesce(nullif(trim(department),''),'(N/A)') as department, COUNT(*) as count
     FROM it_returns
     WHERE is_deleted=0
     GROUP BY coalesce(nullif(trim(department),''),'(N/A)')
     ORDER BY count DESC
     LIMIT 12`
  );

  const byCondition = await dbAll(
    `SELECT coalesce(nullif(trim(condition),''),'(N/A)') as condition, COUNT(*) as count
     FROM it_returns
     WHERE is_deleted=0
     GROUP BY coalesce(nullif(trim(condition),''),'(N/A)')
     ORDER BY count DESC
     LIMIT 12`
  );

  res.json({ ok: true, days, byStatus, byDay, byDepartment, byCondition });
});

itApi.post("/returns", itAuth, itRequireRole("admin"), async (req, res) => {
  await ensureDbReady();
  ...
  const condition_detail = String(req.body?.condition_detail || "").trim();
  const accessories = String(req.body?.accessories || "").trim();

  const returner_name = String(req.body?.returner_name || "").trim();
  const returner_email = String(req.body?.returner_email || "").trim();
  ...
  if (returner_email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(returner_email)) {
    return res.status(400).json({ ok: false, error: "อีเมลผู้คืนไม่ถูกต้อง" });
  }

  const r = await dbRun(
    `INSERT INTO it_returns (
      requester_id, asset_tag, item_name, serial_no,
      condition, condition_detail, accessories,
      returner_name, returner_email, department, location,
      reason, attachment_url, status, requester_ip, received_by
     )
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'PENDING',?,?,?)`,
    [
      req.user.id,
      asset_tag || null, item_name || null, serial_no || null,
      condition || null, condition_detail || null, accessories || null,
      returner_name || null, returner_email || null, department || null, location || null,
  ...
  await itAudit(req.user.id, "RETURN_CREATE", String(r.lastInsertRowid), JSON.stringify({
    asset_tag, item_name, serial_no, returner_name, returner_email, department
  }));

  res.json({ ok: true, id: r.lastInsertRowid });
});

itApi.get("/returns/:id", itAuth, itRequireRole("admin"), async (req, res) => {
  await ensureDbReady();
  const id = Number(req.params.id);
...
itApi.patch("/returns/:id", itAuth, itRequireRole("admin"), async (req, res) => {
  await ensureDbReady();
  const id = Number(req.params.id);
  ...
  if (req.body.condition_detail !== undefined) set("condition_detail", String(req.body.condition_detail || "").trim() || null);
  if (req.body.accessories !== undefined) set("accessories", String(req.body.accessories || "").trim() || null);

  if (req.body.returner_name !== undefined) set("returner_name", String(req.body.returner_name || "").trim() || null);

  if (req.body.returner_email !== undefined) {
    const em = String(req.body.returner_email || "").trim();
    if (em && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(em)) return res.status(400).json({ ok: false, error: "อีเมลผู้คืนไม่ถูกต้อง" });
    set("returner_email", em || null);
  }

  if (req.body.department !== undefined) set("department", String(req.body.department || "").trim() || null);
  if (req.body.location !== undefined) set("location", String(req.body.location || "").trim() || null);
  ...
  if (req.body.completed_at !== undefined) set("completed_at", String(req.body.completed_at || "").trim() || null);

  if (req.body.status !== undefined) {
    const st = String(req.body.status || "PENDING").trim().toUpperCase();
    const allow = ["PENDING","RECEIVED","COMPLETED","CANCELLED"];
    if (!allow.includes(st)) return res.status(400).json({ ok: false, error: "Invalid status" });
    set("status", st);
  }
...
itApi.get("/export/returns.xlsx", itAuth, itRequireRole("admin"), async (req, res) => {
  await ensureDbReady();
  const rows = await dbAll(
    `SELECT r.*, u.name as requester_name, u.email as requester_email
     FROM it_returns r
  ...
  ws.columns = [
    ...
    { header: "Requester(Admin)", key: "requester_name", width: 18 },
    { header: "Requester Email", key: "requester_email", width: 22 },

    { header: "Returner Name", key: "returner_name", width: 18 },
    { header: "Returner Email", key: "returner_email", width: 22 },
    ...
    { header: "Condition Detail", key: "condition_detail", width: 30 },
    { header: "Accessories", key: "accessories", width: 26 },
    ...
    { header: "Recorded By", key: "received_by", width: 18 },
  ...
  res.setHeader("Content-Disposition", `attachment; filename="equipment_returns.xlsx"`);
...
uniApi.post("/requests/:id/reject", async (req, res) => {
  ...
  await sendChat(CHAT_WEBHOOK_APPROVALS, `❌ คำขอ #${id} ถูกปฏิเสธ\nเหตุผล: ${reason}\nโดย: ${actor}`);
